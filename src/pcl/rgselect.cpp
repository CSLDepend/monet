                               ...
                               int min_cluster_sz_value)
{
    ...
    for (auto &cluster : clusters) {
        ...
        float sse_credit = 0, sse_inq = 0;

        for (auto &ind : cluster.indices) {
            pcl::PointXYZRGB &pt = (*cloud)[ind];
            sse_credit += (pt.r/2.55 - avg_credit) * (pt.r/2.55 - avg_credit);
            sse_inq += (pt.g/2.55 - avg_inq) * (pt.g/2.55 - avg_inq);
        }

        float sd_credit = sqrt(sse_credit/(float)(sz - 1)),
              sd_inq = sqrt(sse_inq/(float)(sz - 1));

        s << bucket_number << " " << sz << " "
          << xmin << " " << ymin << " " << zmin << " "
          << xmax - xmin << " " << ymax - ymin << " " << zmax - zmin << " "
          << avg_credit << " " << sd_credit << " "
          << avg_inq << " " << sd_inq << '\n';
    }
...
int main(int argc, char **argv)
{
    ...
    if (type_number == 0) {
        std::cout << make_region_string(rcloud, bucket_number, point_color_threshold_value,
